...
function extractExhaustionScore(raw = {}) {
  return clamp(
    toNumber(
      raw.exhaustionScore ?? raw.autoLevels?.exhaustion?.exhaustionScore,
      0
    ),
    0,
    1
...
export function normalizeSimulationInput(raw = {}) {
  ...
  return {
    ...
    daysForecast,
    simulations,
    exhaustionScore: extractExhaustionScore(raw),
...
export function runMonteCarloSimulation(rawParams) {
  ...
  const riskReward = loss === 0 ? 0 : gain / loss;
  const expectedValue = probTP * gain - probSL * loss;
  const expectedValuePct = expectedValue / p.currentPrice;

  // Calibrated score v2:
  // The old score was too conservative because it ignored pump exhaustion directly
  // and EV% is tiny on small-price tokens. This version rewards exhaustion,
  // directional probability, TP chance, RR quality, and EV quality, while still
  // penalizing stop-loss risk.
  const exhaustionComponent = p.exhaustionScore;
  const rrScore = clamp(riskReward / 3, 0, 1);
  const evScore = clamp(expectedValuePct / 0.08, -1, 1);

  const scoreRaw =
    0.25 * exhaustionComponent +
    0.25 * probDown +
    0.2 * probTP +
    0.15 * rrScore +
    0.1 * evScore -
  ...
  } else if (shortEntryScore >= 0.75 && expectedValue > 0 && probTP > probSL) {
    status = 'STRONG_SHORT_SETUP';
  } else if (shortEntryScore >= 0.65 && expectedValue > 0) {
    status = 'SHORT_VALID';
  } else if (shortEntryScore >= 0.5) {
    status = 'SHORT_WATCH';
  } else if (shortEntryScore >= 0.35) {
    status = 'WEAK_WATCH';
  }
