...
export function normalizeSimulationInput(raw = {}) {
  const currentPrice = Math.max(1, toNumber(raw.currentPrice, 65000));
  const takeProfit = toNumber(raw.takeProfit, currentPrice * 0.96);
  const stopLoss = toNumber(raw.stopLoss, currentPrice * 1.03);
